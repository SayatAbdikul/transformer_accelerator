...
//   intermediate storage plus SystemVerilog math system functions for the first
//   end-to-end baseline SFU bring-up.
...
(
  ...
  localparam real GELU_INV_SQRT2 = 0.70710678118654752440;
  ...
  typedef enum logic [4:0] {
    ...
    F_ROW_WRITE     = 5'd8,
    F_GELU_I8_REQ   = 5'd9,
    F_GELU_I8_LATCH = 5'd10,
    F_GELU_I8_WRITE = 5'd11,
  ...
  function automatic logic [7:0] quantize_to_i8(
    ...
    begin
      if (out_scale_r == 0.0) begin
        ...
        q_i = round_half_even(value_r / out_scale_r);
        if (q_i > 127)
  ...
  function automatic real erf_poly(input real x_r);
    real sign_r;
    real x_abs_r;
    real t_r;
    real poly_r;
    begin
      sign_r = (x_r < 0.0) ? -1.0 : 1.0;
      x_abs_r = (x_r < 0.0) ? -x_r : x_r;
      t_r = 1.0 / (1.0 + 0.3275911 * x_abs_r);
      poly_r = (((((1.061405429 * t_r) - 1.453152027) * t_r + 1.421413741) *
                 t_r - 0.284496736) * t_r + 0.254829592) * t_r;
      erf_poly = sign_r * (1.0 - poly_r * $exp(-(x_abs_r * x_abs_r)));
    end
  endfunction

  function automatic real gelu_real(input real x_r);
    begin
      gelu_real = x_r * 0.5 * (1.0 + erf_poly(x_r * GELU_INV_SQRT2));
    end
  endfunction
        ...
        F_ROW_I8_LATCH: begin
          ...
          for (int lane = 0; lane < 16; lane++) begin
            if ((base_idx + lane) < integer'(n_elems_q))
              row_data_q[base_idx + lane] <= real'(get_i8(sram_b_rdata, lane)) * scale0_q;
          end
          ...
          if (opcode_q == OP_SOFTMAX) begin
            ...
            for (int i = 0; i < SFU_MAX_ROW_ELEMS; i++) begin
              if (i < integer'(n_elems_q)) begin
                exp_r = $exp(row_data_q[i] - row_max_r);
                exp_sum_r = exp_sum_r + exp_r;
              end
            end
            ...
            for (int i = 0; i < SFU_MAX_ROW_ELEMS; i++) begin
              if (i < integer'(n_elems_q)) begin
                exp_r = $exp(row_data_q[i] - row_max_r);
                out_bytes_q[i] <= quantize_to_i8(exp_r / exp_sum_r, scale1_q);
              end
            end
          end else begin
            real sum_r;
            ...
            for (int i = 0; i < SFU_MAX_ROW_ELEMS; i++) begin
              if (i < integer'(n_elems_q))
                sum_r = sum_r + row_data_q[i];
            end
            mean_r = sum_r / real'(n_elems_q);

            var_r = 0.0;
            for (int i = 0; i < SFU_MAX_ROW_ELEMS; i++) begin
              if (i < integer'(n_elems_q))
                var_r = var_r + ((row_data_q[i] - mean_r) * (row_data_q[i] - mean_r));
            end
            var_r = var_r / real'(n_elems_q);
            denom_r = $sqrt(var_r + LN_EPS);

            for (int i = 0; i < SFU_MAX_ROW_ELEMS; i++) begin
              real y_r;
              if (i < integer'(n_elems_q)) begin
                y_r = ((row_data_q[i] - mean_r) / denom_r) * gamma_q[i] + beta_q[i];
